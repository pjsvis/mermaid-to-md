...
// ── Style stubs (ratatui extraction) ─────────────────────────────
// The original code used ratatui::style::Style for styled terminal output.
// We only need the plain_lines path (Vec<String>) for the CLI, so these
// are no-op placeholders that satisfy the type system without the dependency.
...
fn render_class(
    ...
    let plain_lines = canvas.to_lines(styles);
    Ok(MermaidArt {
        plain_lines,
    })
}
...
impl Canvas {
    ...
    fn to_lines(&self, _styles: &MermaidStyles) -> Vec<String> {
...
// style_for removed — plain_lines path doesn't need style mapping.
