...
* **Locus tags** (`[LOC: phase]`, `[WAYPOINT: milestone]`): lightweight markers that section a multi-phase dialog. `[LOC:]` marks a phase transition (orient/analyze/edit/verify, or a shift of file/concern); `[WAYPOINT:]` marks a completed milestone. **Consumers:** human navigation of long dialogs (present) + per-section compaction into the handoff/review (future — see `briefs/2026-07-14-brief-locus-tag-consumer.md`). **Anti-ceremony:** omit on single-phase turns — a tag without underlying structure is entropy, not anti-entropy.
